    ...
    // Build Process Defaults (editable controls)
        ...
        statusItems.push(
            <div className="build-status-item stacked" key="back-construction">
                ...
                <div className="content-block">
                    <select className="input compact inline-control" value={renderBr.backConstruction?.id ?? ''} title="Back construction"
                    ...
                    {(renderBr.backConstruction?.id==='dado-thick-back' || renderBr.backConstruction?.id==='inset-nailer-dado-back') && (
                        <input className="input input-narrow compact inline-control" type="number" step="0.01" title="Back dado depth (in)" value={renderBr.backConstruction?.dadoDepthIn ?? 0.25}
                            onChange={(e)=> setBuildResult(prev => ({
                                ...(prev ?? { units: 'in', materials: { case: {} as any, hardware: {} as any }, toeKick: { attached: null } } as BuildResult),
                            ...
                            {!!renderBr.dados?.blind?.enabled && (
                                <>
                                    <label className="mr-2 inline-label">Blind offset
                                        <input className="input input-narrow compact inline-control ml-1" type="number" step="0.01" placeholder="Blind offset (in)" title="Blind offset (in)" value={renderBr.dados?.blind?.offsetIn ?? 0.25}
                                        onChange={(e)=> setBuildResult(prev => {
                                            const base = prev ?? { units: 'in', materials: { case: {} as any, hardware: {} as any }, faceFramePositioning: renderBr.faceFramePositioning, backConstruction: renderBr.backConstruction, dados: { typeId: renderBr.dados?.typeId ?? 'full-thickness-dados' }, toeKick: renderBr.toeKick } as BuildResult;
                                            return {
                                                ...base,
        ...
        statusItems.push(
            <div className="build-status-item stacked" key="toe-kick">
                ...
                <div className="content-block">
                    <select className="input compact inline-control" value={t.attached==null ? '' : (t.attached ? 'yes' : 'no')} title="Toe kick attached"
                        onChange={(e)=> setBuildResult(prev => ({
                                ...(prev ?? { units: 'in', materials: { case: {} as any, hardware: {} as any }, toeKick: { attached: null } } as BuildResult),
                    ...
                    >
                        ...
                        <>
                                                                <div className="inline-row">
                                                                    ...
                                                                    <input
                                                                        className="input input-narrow compact inline-control ml-1"
                                                                        type="number"
                                                                        step="0.01"
