...
function inferCostUnit(item: SOMItem): CostUnit {
  ...
  if (item.category === "Hardware") return item.subtype === "DrawerSlides" ? "perPair" : "each";
  return "each";
}
...
function useOutsideClose(ref: React.RefObject<HTMLElement>, onClose: () => void) {
  useEffect(() => {
    const h = (e: MouseEvent) => { if (!ref.current || ref.current.contains(e.target as Node)) return; onClose(); };
  ...
  return (
    <div
      ...
      style={{
        ...
        background: "#121821", border: "1px solid #283241", borderRadius: 8, boxShadow: "0 8px 24px rgba(0,0,0,.35)", padding: 6, minWidth: 180,
      }}
    >
...
function SOMModal({
...
}: { initial: Partial<SOMItem>; onSave: (v: SOMItem) => void; onCancel: () => void; defaultCategory: Category; }) {
  ...
  /* friendly inch strings */
  const [widthInStr, setWidthInStr] = useState(v.widthMm != null ? formatInchesMixed(v.widthMm) : "");
  const [lengthInStr, setLengthInStr] = useState(v.lengthMm != null ? formatInchesMixed(v.lengthMm) : "");
  const [thicknessInStr, setThicknessInStr] = useState(v.thicknessMm != null ? formatInchesMixed(v.thicknessMm) : "");
  const [sideInStr, setSideInStr] = useState(v.sideClearanceMm != null ? formatInchesMixed(v.sideClearanceMm) : "");
  const [bottomInStr, setBottomInStr] = useState(v.bottomClearanceMm != null ? formatInchesMixed(v.bottomClearanceMm) : "");
  const [topInStr, setTopInStr] = useState(v.topClearanceMm != null ? formatInchesMixed(v.topClearanceMm) : "");
  const [hLenInStr, setHLenInStr] = useState(v.hardwareLengthMm != null ? formatInchesMixed(v.hardwareLengthMm) : "");
  const [offsetInStr, setOffsetInStr] = useState(v.hingePlateOffsetMm != null ? formatInchesMixed(v.hingePlateOffsetMm) : "");
  const [bandingFeetStr, setBandingFeetStr] = useState(
    v.category === "Banding" && v.lengthMm != null ? String(Math.round((v.lengthMm / 304.8) * 100) / 100) : ""
  );

  /* in<->mm sync */
  function handleWidthInChange(s: string) { setWidthInStr(s); const d = parseInchesTextToDecimal(s); setV(c => ({ ...c, widthMm: d==null?undefined:floorToHalfMm(d*25.4) })); }
  function handleWidthMmChange(raw: string) { const n = raw?Number(raw):NaN; const mm = isFinite(n)?floorToHalfMm(n):undefined; setV(c=>({ ...c, widthMm:mm })); setWidthInStr(mm==null?"":formatInchesMixed(mm)); }
  function handleWidthInBlur() { setWidthInStr(s => normalizeInchesTextFloor16(s)); }

  function handleLengthInChange(s: string) { setLengthInStr(s); const d=parseInchesTextToDecimal(s); setV(c=>({ ...c, lengthMm: d==null?undefined:floorToHalfMm(d*25.4) })); }
  function handleLengthMmChange(raw: string) { const n=raw?Number(raw):NaN; const mm=isFinite(n)?floorToHalfMm(n):undefined; setV(c=>({ ...c, lengthMm:mm })); setLengthInStr(mm==null?"":formatInchesMixed(mm)); }
  function handleLengthInBlur() { setLengthInStr(s => normalizeInchesTextFloor16(s)); }

  function handleBandingFeetChange(s: string) {
    setBandingFeetStr(s);
    const n = s === "" ? NaN : Number(s);
    const mm = isFinite(n) ? Math.max(0, n) * 304.8 : undefined;
    setV(c => ({ ...c, lengthMm: mm==null?undefined:floorToHalfMm(mm) }));
  }

  function handleThicknessInChange(s: string) { setThicknessInStr(s); const d=parseInchesTextToDecimal(s); const mm=d==null?undefined:d*25.4; setV(c=>({ ...c, thicknessMm: mm==null?undefined:floorToHalfMm(mm) })); }
  function handleThicknessMmChange(raw: string) { const n=raw?Number(raw):NaN; const mm=isFinite(n)?floorToHalfMm(n):undefined; setV(c=>({ ...c, thicknessMm:mm })); setThicknessInStr(mm==null?"":formatInchesMixed(mm)); }
  function handleThicknessInBlur() { setThicknessInStr(s => normalizeInchesTextFloor16(s)); }

  function handleSideInChange(s: string) { setSideInStr(s); const d=parseInchesTextToDecimal(s); setV(c=>({ ...c, sideClearanceMm: d==null?undefined:floorToHalfMm(d*25.4) })); }
  function handleSideMmChange(raw: string) { const n=raw?Number(raw):NaN; const mm=isFinite(n)?floorToHalfMm(n):undefined; setV(c=>({ ...c, sideClearanceMm:mm })); setSideInStr(mm==null?"":formatInchesMixed(mm)); }
  function handleSideInBlur() { setSideInStr(s => normalizeInchesTextFloor16(s)); }

  function handleBottomInChange(s: string) { setBottomInStr(s); const d=parseInchesTextToDecimal(s); setV(c=>({ ...c, bottomClearanceMm: d==null?undefined:floorToHalfMm(d*25.4) })); }
  function handleBottomMmChange(raw: string) { const n=raw?Number(raw):NaN; const mm=isFinite(n)?floorToHalfMm(n):undefined; setV(c=>({ ...c, bottomClearanceMm:mm })); setBottomInStr(mm==null?"":formatInchesMixed(mm)); }
  function handleBottomInBlur() { setBottomInStr(s => normalizeInchesTextFloor16(s)); }

  function handleTopInChange(s: string) { setTopInStr(s); const d=parseInchesTextToDecimal(s); setV(c=>({ ...c, topClearanceMm: d==null?undefined:floorToHalfMm(d*25.4) })); }
  function handleTopMmChange(raw: string) { const n=raw?Number(raw):NaN; const mm=isFinite(n)?floorToHalfMm(n):undefined; setV(c=>({ ...c, topClearanceMm:mm })); setTopInStr(mm==null?"":formatInchesMixed(mm)); }
  function handleTopInBlur() { setTopInStr(s => normalizeInchesTextFloor16(s)); }

  function handleHLenInChange(s: string) { setHLenInStr(s); const d=parseInchesTextToDecimal(s); setV(c=>({ ...c, hardwareLengthMm: d==null?undefined:floorToHalfMm(d*25.4) })); }
  function handleHLenMmChange(raw: string) { const n=raw?Number(raw):NaN; const mm=isFinite(n)?floorToHalfMm(n):undefined; setV(c=>({ ...c, hardwareLengthMm:mm })); setHLenInStr(mm==null?"":formatInchesMixed(mm)); }
  function handleHLenInBlur() { setHLenInStr(s => normalizeInchesTextFloor16(s)); }

  function handleOffsetInChange(s: string) { setOffsetInStr(s); const d=parseInchesTextToDecimal(s); setV(c=>({ ...c, hingePlateOffsetMm: d==null?undefined:floorToHalfMm(d*25.4) })); }
  function handleOffsetMmChange(raw: string) { const n=raw?Number(raw):NaN; const mm=isFinite(n)?floorToHalfMm(n):undefined; setV(c=>({ ...c, hingePlateOffsetMm:mm })); setOffsetInStr(mm==null?"":formatInchesMixed(mm)); }
  function handleOffsetInBlur() { setOffsetInStr(s => normalizeInchesTextFloor16(s)); }

  useEffect(() => { setV(cur => ({ ...cur, costUnit: inferCostUnit(cur) })); }, [v.category, v.subtype]);
    ...
    <div className={styles.modalBg} onClick={onCancel}>
      ...
      >
        ...
        <div style={{ display:"flex", justifyContent:"end", gap: 8, marginTop: 8 }}>
          ...
          <button ref={saveBtnRef} className={`${styles.btn} ${styles.btnGreen}`}
            onClick={() => { const t=v.thicknessMm; onSave({ ...v, thicknessMm: t==null?t:floorToHalfMm(t) }); }}>
            Save
          </button>
        </div>
      </div>
...
/* ===== grid templates ensure exact alignment ===== */
function gridTemplateFor(active: Category, sub?: HardwareSubtype) {
  if (active === "Hardware") {
    // Name | Subtype | SKU | Vendor | Side | Bottom | Len/Offset | Cost
    return "360px 120px 160px 160px 140px 160px 160px 110px";
  }
  // Material | SKU | Width | Length | Thick | WidthM | LengthM | ThickM | Cost
  return "360px 200px 90px 110px 90px 90px 90px 90px 110px";
}
...
export default function SOMPage() {
  ...
  function onImport() {
    ...
    inp.onchange = async () => {
      ...
      try {
        const items = parseCsv(text).map((v) => ({ ...(v.id ? { id: v.id } : { id: uid() }), ...v }));
        setRows(items);
  ...
  /* headers as grid */
  function HeaderRow() {
    const tmpl = gridTemplateFor(activeSomTab, sub);
    if (activeSomTab === "Hardware") {
      return (
        <div className={styles.gridHeader} style={{ gridTemplateColumns: tmpl }}>
          <div>Name</div><div>Subtype</div><div>SKU</div><div>Vendor</div>
          <div>Side Clr (in/mm)</div><div>Bottom Clr (in/mm)</div><div>Len/Offset (in/mm)</div><div>Cost</div>
        </div>
      );
    }
    return (
      <div className={styles.gridHeader} style={{ gridTemplateColumns: tmpl }}>
        <div>Material</div><div>SKU</div><div>Width</div><div>Length</div><div>Thick</div>
        <div>WidthM</div><div>LengthM</div><div>ThickM</div><div>Cost</div>
      </div>
    );
  }

  function DataRow({ r }: { r: SOMItem }) {
    const tmpl = gridTemplateFor(activeSomTab, sub);
    if (r.category === "Hardware") {
      return (
        <div
          className={`${styles.gridRow} ${sel===r.id?styles.rowSelected:""}`}
        ...
        >
          <div className={styles.cell}>{r.name}</div>
          <div className={styles.cell}>{r.subtype ?? ""}</div>
          <div className={styles.cell}>{r.sku ?? ""}</div>
          <div className={styles.cell}>{r.vendor ?? ""}</div>
          <div className={styles.cell}>{r.subtype === "DrawerSlides" ? <InMmPair inches={formatInchesMixed(r.sideClearanceMm)} mm={displayMm0_5(r.sideClearanceMm)} /> : "-"}</div>
          <div className={styles.cell}>{r.subtype === "DrawerSlides" ? <InMmPair inches={formatInchesMixed(r.bottomClearanceMm)} mm={displayMm0_5(r.bottomClearanceMm)} /> : "-"}</div>
          <div className={styles.cell}>{r.subtype === "HingePlates" ? <InMmPair inches={formatInchesMixed(r.hingePlateOffsetMm)} mm={displayMm0_5(r.hingePlateOffsetMm)} /> : <InMmPair inches={formatInchesMixed(r.hardwareLengthMm)} mm={displayMm0_5(r.hardwareLengthMm)} />}</div>
          <div className={styles.cell}>{renderCost(r)}</div>
        </div>
      );
    }
    return (
      <div
        className={`${styles.gridRow} ${sel===r.id?styles.rowSelected:""}`}
        style={{ gridTemplateColumns: gridTemplateFor(activeSomTab, sub) }}
        onClick={()=>setSel(r.id)}
        onDoubleClick={()=>setShowModal(r)}
        onContextMenu={(e)=>{ e.preventDefault(); setSel(r.id); setMenu({ x:e.clientX, y:e.clientY, id:r.id }); }}
      >
        <div className={styles.cell}>{r.name}</div>
        <div className={styles.cell}>{r.sku ?? ""}</div>
        <div className={styles.cell}>{formatInchesMixed(r.widthMm)}</div>
        <div className={styles.cell}>{r.category==="Banding" ? formatFeet(r.lengthMm) : formatInchesMixed(r.lengthMm)}</div>
        <div className={styles.cell}>{formatInchesMixed(r.thicknessMm)}</div>
        <div className={styles.cell}>{displayMm0_5(r.widthMm)}</div>
        <div className={styles.cell}>{displayMm0_5(r.lengthMm)}</div>
        <div className={styles.cell}>{displayMm0_5(r.thicknessMm)}</div>
        <div className={styles.cell}>{renderCost(r)}</div>
      </div>
    );
  }
  ...
  return (
    <div className={styles.wrap}>
      <div className="page-header page-header-som"></div>
      <TopNav active="SOM" />
...
{activeSomTab === "Hardware" && (
  <div className={styles.filterItem}>
    ...
    <select
      ...
      value={sub ?? ""}                               // "" when showing all
      onChange={(e) => {
        const val = e.target.value as HardwareSubtype | "";
        setSub(val === "" ? undefined : (val as HardwareSubtype));
      }}
    >
      <option value="">None</option>                  {/* show all hardware */}
      <option value="DrawerSlides">Drawer Slides</option>
      <option value="Hinges">Hinges</option>
      <option value="HingePlates">Hinge Plates</option>
    </select>
        ...
        <div className={styles.gridTable}>
          ...
          <div className={styles.gridBody}>
            {filtered.map((r) => <DataRow key={r.id} r={r} />)}
          </div>
        </div>
      </div>
